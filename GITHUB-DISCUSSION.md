...
2. A broader observation: most of the adapter I wrote is working around **Node-runtime
   ...
   interestingly, would also **improve the plain Node (serverful) story**. I'd like to gauge
   appetite for exploring it.
...
- **Direct URL hit** → the server runs it during SSR (`serverFetcher`).
- **Client-side navigation into the route** → the browser runs it (`clientFetcher`).
...
- **Read the resource directly on the server** (e.g. `import data from …`) — only works when the
...
- **Computed handler** (`servedAt` timestamp) proves it's the real handler running, not a static read.
- **Cookie forwarding** — a self-call to `/api/whoami` sees the inbound `uid` cookie.
- **Third-party endpoint hit directly** from `serverFetcher` — works, correctly *not* rerouted.
...
"context-forwarding on a self-call" is **already a latent footgun on Node today** — it's not a
...
**I prefer D** (identical call sites; cleanest expression of the isomorphism) and am shipping
...
The proposal: make core runtime-agnostic and delegate environment concerns to a small
**Runtime Adapter interface**, with **Node as the default adapter**. Workers/Deno/Bun become
adapters, not forks.

Candidate extension points:

- **`renderToStream`** — Node → pipeable; edge → `renderToReadableStream`. Core stops importing `react-dom/server` directly.
- **`assetManifest` / `serveStatic`** — adapter supplies the loadable-stats + CSS manifest from memory/import, so core never does per-request `fs`.
- **`createServer` + middleware** — adapter owns the transport; middleware is app-supplied, not baked.
- **`dispatchSelf` + `ctx.fetch`** — the isomorphism primitive from Challenge 1.
- **`logger`, `config`, `telemetry`** — injected, not global; sensible edge/container defaults.

### Why this is worth it even if you never touch Workers

Framing this as "Workers-only" undersells it. Each item is a **serverful improvement**:

1. **Kill per-request `fs`** for build manifests → lower p99 (blocking I/O out of the render hot path).
2. **`ctx.fetch` that forwards request context** → fixes the silent cookie/authz drop on self-calls that exists on Node today.
3. **In-process self-dispatch** → no loopback socket / double middleware for server-side data fetching.
4. **stdout logger by default** → correct for containers/12-factor; file transport opt-in.
5. **Build-time alias resolution** (vs the `module-alias` runtime require-patch) → smaller/faster, works with any bundler, ESM-safe.
6. **Optional/lazy telemetry** (no unconditional `@opentelemetry/*` requires) → smaller bundles, faster cold start.
7. **Reproducible builds** (no absolute build-host paths baked in) → Docker cache, relocatable artifacts.
