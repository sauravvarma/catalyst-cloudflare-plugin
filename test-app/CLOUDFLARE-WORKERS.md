...
|---|---|---|---|---|---|---|
| A | ✅ | ✗ (abs vs rel) | ✅ | ✗ | none | none (but broken on Workers) |
...
- **S9** shows C2's transparent forwarding makes Workers behave differently from vanilla Node
  ...
  both runtimes.
- **S2/S3** show naive self-dispatch loses request context/body unless the adapter forwards it,   the adapter, not the app, should own this.
...
**Highest-leverage first step:** the data-fetch primitive (`ctx.fetch` + in-process dispatch), it resolves the C2/D isomorphism question *and* fixes a real serverful bug. See
